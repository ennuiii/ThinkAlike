...
const GameBuddiesReturnButton: React.FC<GameBuddiesReturnButtonProps> = ({
  ...
  const handleReturn = () => {
    console.log('[GameBuddies] Return all players clicked');
    setIsReturning(true);
  ...
  // 1. Not launched from GameBuddies, OR
  // 2. Not the host (only host can return all players)
  if (!isGameBuddiesLaunched || !isHost) {
    return null;
  }
  ...
  return (
    <div className="gamebuddies-return">
      ...
      >
        ← Return All Players to GameBuddies
      </button>
    </div>
  );
};
